...
import numpy as np
from core import ViTModelWrapper, create_index, get_dominant_color, get_texture_vector
from database import DatabaseManager

# Set page config
st.set_page_config(page_title="Visual Hybrid Image Search (Google ViT)", layout="wide")

st.title("🛍️ Hybrid Reverse Image Search")
st.markdown("Powered by **Google ViT-Base** (768-dim) + **Color & Texture Analysis**.")

# Paths
PARENT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
IMAGES_DIR = os.path.join(PARENT_DIR, "Images")
MODEL_DIR = os.path.join(PARENT_DIR, "models", "googlevit-base-patch16-224-in21k")

@st.cache_resource
def load_model():
    return ViTModelWrapper(MODEL_DIR)
...
try:
    ...
    st.info("Check if model path exists and transformers/torch are installed.")
    st.stop()
...
if st.sidebar.button("🔄 Update DB Index"):
    st.sidebar.warning("Processing collection with Google ViT (768-dim)...")
...
st.sidebar.markdown("---")
st.sidebar.caption("Google ViT-Base-Patch16-224 | 768 Dimensions")
...
if uploaded_file is not None:
    ...
    with col2:
        ...
        with st.spinner("Executing Google ViT hybrid search..."):
            ...
            if query_emb is not None:
                ...
                if results:
                    st.success(f"Matched {len(results)} items using Google ViT-Base Engine")
                    ...
                    with f_col2:
                        ...
                        st.caption(f"ViT Pattern: {p_score:.2f} | Color: {c_score:.2f} | Texture: {t_score:.2f}")
